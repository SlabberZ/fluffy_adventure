...
lab2.factory('processMove', function() {
  return function() {
    self.checkWins =  function(player1_choice, player2_choice) {
      ...
      if (angular.equals(a, b)) return 0;
      else if ((angular.equals(b, 0) && angular.equals(a, 1)) ||
               (angular.equals(b, 1) && angular.equals(a, 2)) ||
               (angular.equals(b, 2) && angular.equals(a, 0))) return 1;
      else if ((angular.equals(a, 0) && angular.equals(b, 1)) ||
               (angular.equals(a, 1) && angular.equals(b, 2)) ||
               (angular.equals(a, 2) && angular.equals(b, 0))) return 2;
      /* return 0 if tie */
  ...
  $scope.onNext = function (choice) {
    ...
    // Additionally check if player 2 has best 2/3
    {
      ...
      if(window.plugins && window.plugins.nativepagetransitions) {
        ...
        $state.go("player2", {
          'player1_choice': player1.getChoiceAsNumber(choice),
...
lab2.controller('player2', function(processMove, $rootScope, $scope, $ionicModal, $ionicLoading, $ionicViewSwitcher, $state, $stateParams, navigation) {
  ...
  $scope.onNext = function (choice) {
    ...
    $scope.player2_choice = process.getChoiceAsNumber(choice);
    /* TODO: Use processMove factory instance to check if player1 has a win */
    var winner = process.checkWins($stateParams.player1_choice, $scope.player2_choice);
    /* TODO: Use processMove factory instance to check if player2 has a win */
    if (winner == 1) {
      $scope.player1_wins = $scope.player1_wins + 1;
    }
    else if (winner == 2) {
      $scope.player2_wins = $scope.player2_wins + 1;
    }

    if ($scope.player1_wins >= 2) // Check if player1 has best 2/3 // changed from stateParams to scope
    {
      if(window.plugins && window.plugins.nativepagetransitions) {
        // Ignore this block for now; this is for native transitions (More on Wednesday 2/22/17) //
      }
    ...
    else if ($scope.player2_wins >= 2) // Check if player2 has best 2/3 // changed from stateParams to scope
    {
      if(window.plugins && window.plugins.nativepagetransitions) {
        // Ignore this block for now; this is for native transitions (More on Wednesday 2/22/17) //
      }
